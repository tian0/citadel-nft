...
contract Citadel is ERC721Full, Ownable, Pausable {
    ...
    /// @notice Remove member by setting enrolled to false, whitelist to false, deleting from members array, then burn tokenId
    /// @dev Only contract owner can remove member
    /// @param _member The member address or adresses to be removed
    function remove(address[] memory _member)
    public 
    onlyOwner()
    {
        for (uint i = 0; i < _member.length; i++)
        {
            uint _token = tokenId[_member[i]];
            enrolled[_member[i]] = false;
            whiteList[_member[i]] = false;
            delete members[_token];
            _burn(_member[i], tokenId[_member[i]]);
            emit Remove(_member[i]);
        } 
    }
